# --- ФАЙЛ: main.py (ФІНАЛЬНА, СПРОЩЕНА ВЕРСІЯ) ---
...
DB_NAME = "debt_book_v2.db" # Не використовується, але залишено для сумісності
...
# --- 🗃️ КЛАСС ДЛЯ РАБОТЫ С БАЗОЙ ДАННЫХ (СПРОЩЕНА ВЕРСІЯ) ---
class Database:
    def __init__(self, conn_url):
        ...
        try:
            # НАЙПРОСТІШИЙ І НАДІЙНИЙ СПОСІБ ПІДКЛЮЧЕННЯ
...
# (Тут іде весь ваш код без змін: group_only, escape_markdown, calculate_balances, всі хендлери і т.д.)
def group_only(func):
    @wraps(func)
...
async def add_debt_get_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: context.user_data['amount'] = float(update.message.text.replace(',', '.')); msg_id = context.user_data.get('prompt_msg_id'); await update.message.delete();
...
async def repay_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try: amount = float(update.message.text.replace(',', '.')); await update.message.delete();
