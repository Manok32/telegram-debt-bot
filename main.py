...
# DB_NAME = "debt_book_v2.db" # Это, кажется, осталось от SQLite версии, для PostgreSQL не используется напрямую

# --- 🎨 ЭМОДЗИ И СТРОКИ ---
EMOJI = {
    "money": "💰", "repay": "💸", "split": "🍕", "status": "📊",
    "my_debts": "👤", "history": "📜", "ok": "✅", "cancel": "❌",
    "back": "↩️", "user": "👤", "warning": "⚠️", "party": "🎉", "lock": "🔒"
}
...
class Database:
    def __init__(self, conn_url): # Изменено init на __init__
        if not conn_url:
            raise ValueError("DATABASE_URL не найден. Убедитесь, что он добавлен в Environment Variables.")
        
        logger.info("Connecting to PostgreSQL database with SSL require...")
        try:
            ...
            logger.critical(f"!!! КРИТИЧНА ПОМИЛКА ПІДКЛЮЧЕННЯ ДО БАЗИ: {e}")
            raise
...
db = Database(DATABASE_URL) # Инициализация объекта базы данных
...
def group_only(func):
    ...
    async def wrapped(update: Update, context: ContextTypes.DEFAULT_TYPE, *args, **kwargs):
        if not update.effective_chat or update.effective_chat.type == "private":
            ...
            return ConversationHandler.END if 'conv' in str(func.qualname) else None # Проверяем, если это часть ConversationHandler
...
def get_user_mention(user_id, chat_id):
    ...
    # Используем escape_markdown, чтобы имена пользователей с символами Markdown не ломали форматирование
...
async def process_final_step(update: Update, context: ContextTypes.DEFAULT_TYPE, db_action):
    ...
    try:
        ...
        logger.error(f"Помилка при записі в базу: {e}")
    ...
    if prompt_msg_id:
        try:
            ...
            # Если сообщение уже удалено или не существует
            pass
    await start_menu(update, context)
    return ConversationHandler.END
...
async def add_debt_select_creditor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    context.user_data['creditor_id'] = int(query.data.split('_')[1]) # Исправлено query.data.split('')[1]
    members = db.get_group_members(query.message.chat_id)
    keyboard = [[InlineKeyboardButton(name, callback_data=f"user_{uid}")] for uid, name in members if uid != context.user_data['creditor_id']] # Исправлено f"user{uid}"
    keyboard.append([InlineKeyboardButton(f"{EMOJI['back']} Вернуться в меню", callback_data="cancel")])
...
async def add_debt_get_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.delete() # Удаляем сообщение пользователя с суммой

        if msg_id:
            await context.bot.edit_message_text(
                "За что? (Комментарий или /skip для пропуска)",
...
async def add_debt_save(update: Update, context: ContextTypes.DEFAULT_TYPE, is_skip=False):
    ...
    await update.message.delete() # Удаляем сообщение пользователя с комментарием
    def action():
        db.add_transaction(
            update.effective_chat.id,
            context.user_data['creditor_id'],
...
async def repay_select_debtor(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    context.user_data['debtor_id'] = int(query.data.split('_')[1]) # Исправлено query.data.split('')[1]
    members = db.get_group_members(query.message.chat_id)
    keyboard = [[InlineKeyboardButton(name, callback_data=f"user_{uid}")] for uid, name in members if uid != context.user_data['debtor_id']] # Исправлено f"user{uid}"
    keyboard.append([InlineKeyboardButton(f"{EMOJI['back']} Вернуться в меню", callback_data="cancel")])
...
async def repay_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        def action():
            ...
            # Тот, кто возвращает (debtor_id в контексте диалога), становится "кредитором"
            # в этой транзакции, так как он ОТДАЕТ деньги (его баланс уменьшается).
            # Тот, кому возвращают (creditor_id в контексте диалога), становится "должником",
            # так как он ПОЛУЧАЕТ деньги (его баланс увеличивается).
            creditor_for_this_transaction = context.user_data['debtor_id']
            debtor_for_this_transaction = context.user_data['creditor_id']
            db.add_transaction(
                update.effective_chat.id,
                creditor_for_this_transaction,
                debtor_for_this_transaction,
                amount,
                "Погашение долга"
            )
        return await process_final_step(update, context, action)
...
async def status_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text(
        text,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(f"{EMOJI['back']} Назад в меню", callback_data="back_to_menu")]]),
...
async def my_debts_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for (d_id, c_id), amount in net_debts.items():
        if d_id == user_id:
            i_owe += f" • {get_user_mention(c_id, chat_id)}: {escape_markdown(f'{amount:.2f}')} UAH\n"
        if c_id == user_id:
            owe_me += f" • {get_user_mention(d_id, chat_id)}: {escape_markdown(f'{amount:.2f}')} UAH\n"
...
async def history_menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Извлекаем уникальные пары год-месяц и сортируем их
    months = sorted(list({t[5].strftime("%Y-%m") for t in transactions}), reverse=True)
    keyboard = []
    for m in months:
        dt_object = datetime.strptime(m, '%Y-%m')
        keyboard.append([InlineKeyboardButton(f"{RUSSIAN_MONTHS_NOM[dt_object.month]} {dt_object.year}", callback_data=f"history_{m}")])

    await query.message.edit_text(
        "Выберите месяц:",
        reply_markup=InlineKeyboardMarkup(keyboard + [[InlineKeyboardButton(f"{EMOJI['back']} Назад в меню", callback_data="back_to_menu")]])
    )
    await query.answer()
...
def main():
    ...
    fallbacks = [
        CallbackQueryHandler(end_conversation, pattern="^cancel$"),
    ...
    add_debt_handler = ConversationHandler(
        ...
        states={
            ...
            GET_COMMENT: [
                CommandHandler('skip', lambda u, c: add_debt_save(u, c, True)),
                MessageHandler(filters.TEXT & ~filters.COMMAND, add_debt_save)
            ]
        },
        fallbacks=fallbacks,
        per_user=False, # Может быть True, зависит от логики, но для группы False часто удобнее
        per_chat=True,
        allow_reentry=True
    )
    ...
    split_handler = ConversationHandler(
        ...
        states={
            ...
            SPLIT_GET_COMMENT: [
                CommandHandler('skip', lambda u, c: split_save(u, c, True)),
                MessageHandler(filters.TEXT & ~filters.COMMAND, split_save)
            ]
        },
        fallbacks=fallbacks,
        per_user=False,
...
if __name__ == "__main__":
    logger.info("Запуск веб-сервера для поддержания активности...")
    flask_thread = Thread(target=run_flask)
    flask_thread.daemon = True
    flask_thread.start()

    logger.info("Запуск пінгу бази даних для підтримки активності...")
